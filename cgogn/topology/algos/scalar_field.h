...
public:
		...
		vertex_type_computed_(false)
	{
		...
		vertex_type_ = map.template attribute<CriticalPoint::Type, Vertex::ORBIT>(name);
	}
...
public:
	...
	void extract_ascending_3_manifold(std::vector<Vertex>& boundary)
	{
		...
		VertexAttribute<uint32> manifold_id = map_.template attribute<uint32, Vertex::ORBIT>("manifold_id");
