...
#ifndef CGOGN_MODELING_TILING_SQUARE_CYLINDER_H_
#define CGOGN_MODELING_TILING_SQUARE_CYLINDER_H_
...
public:
	CGOGN_NOT_COPYABLE_NOR_MOVABLE(SquareCylinder);
	//@{
	...
	class CylinderTopo: public SquareGrid<INNERMAP>::template GridTopo<INNERMAP>
	{
	public:
		CylinderTopo(Tiling<INNERMAP>* c, uint32 n, uint32 z):
			SquareGrid<INNERMAP>::template GridTopo<INNERMAP>(c,n,z)
		{
...
public:
	SquareCylinder(MAP& map, uint32 n, uint32 z, bool top_closed, bool bottom_closed):
		Tiling<MAP>(map),
	...
	SquareCylinder(MAP& map, unsigned int n, unsigned int z):
		SquareCylinder<MAP>(map, n, z, true, true)
	{}
	...
	void embed_into_sphere(typename MAP::template VertexAttribute<T>& attribute,
						   float32 radius)
	{
		const float32 alpha = 2.f * float32(M_PI) / float32(this->nx_);
		const float32 beta = float32(M_PI) / float32(this->ny_ + 2);
...
extern template class CGOGN_MODELING_EXPORT SquareCylinder<CMap2>;
...
#endif // CGOGN_MODELING_TILING_SQUARE_CYLINDER_H_
