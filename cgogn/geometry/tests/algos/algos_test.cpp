...
{
	{
		...
		Dart t;
		map.foreach_dart_until([&t] (Dart d) { t = d; return false; });
		const StdArray centroid = cgogn::geometry::centroid<StdArray>(map, CMap2::Face(t), vertex_position);
	...
	}
	{
		...
		Dart t;
		map.foreach_dart_until([&t] (Dart d) { t = d; return false; });
		const EigenVec3d centroid = cgogn::geometry::centroid<EigenVec3d>(map, CMap2::Face(t), vertex_position);
		EXPECT_DOUBLE_EQ(centroid[0], 5.0/3.0);
		EXPECT_DOUBLE_EQ(centroid[1], 5.0/3.0);
		EXPECT_DOUBLE_EQ(centroid[2], 0);
	}
...
{
	{
		...
		Dart q;
		map.foreach_dart_until([&q] (Dart d) { q = d; return false; });
		const StdArray centroid = cgogn::geometry::centroid<StdArray>(map, CMap2::Face(q), vertex_position);
	...
	}
	{
		...
		Dart q;
		map.foreach_dart_until([&q] (Dart d) { q = d; return false; });
		const EigenVec3d centroid = cgogn::geometry::centroid<EigenVec3d>(map, CMap2::Face(q), vertex_position);
		EXPECT_DOUBLE_EQ(centroid[0], 2.5);
		EXPECT_DOUBLE_EQ(centroid[1], 1);
		EXPECT_DOUBLE_EQ(centroid[2], 0);
	}
...
{
	{
		...
		Dart t;
		map.foreach_dart_until([&t] (Dart d) { t = d; return false; });
		const StdArray& n1 = cgogn::geometry::triangle_normal<StdArray>(map, CMap2::Face(t), vertex_position);
		const StdArray& n2 = cgogn::geometry::face_normal<StdArray>(map, CMap2::Face(t), vertex_position);
	...
	}
	{
		...
		Dart t;
		map.foreach_dart_until([&t] (Dart d) { t = d; return false; });
		const EigenVec3d& n1 = cgogn::geometry::triangle_normal<EigenVec3d>(map, CMap2::Face(t), vertex_position);
		const EigenVec3d& n2 = cgogn::geometry::face_normal<EigenVec3d>(map, CMap2::Face(t), vertex_position);
		EXPECT_TRUE(cgogn::almost_equal_relative(n1[0], n2[0]));
		EXPECT_TRUE(cgogn::almost_equal_relative(n1[1], n2[1]));
		EXPECT_TRUE(cgogn::almost_equal_relative(n1[2], n2[2]));
...
{
	{
		...
		Dart q;
		map.foreach_dart_until([&q] (Dart d) { q = d; return false; });
		const StdArray& n1 = cgogn::geometry::face_normal<StdArray>(map, CMap2::Face(q), vertex_position);
		const StdArray& cross = n1.cross(StdArray(0.,0.,1.));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[0], 0.));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[1], 0.));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[2], 0.));
	}
	{
		...
		Dart q;
		map.foreach_dart_until([&q] (Dart d) { q = d; return false; });
		const EigenVec3d& n1 = cgogn::geometry::triangle_normal<EigenVec3d>(map, CMap2::Face(q), vertex_position);
		const EigenVec3d& cross = n1.cross(EigenVec3d(0,0,1));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[0], 0.));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[1], 0.));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[2], 0.));
