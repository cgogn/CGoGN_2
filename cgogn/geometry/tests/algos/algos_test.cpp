...
{
	{
		...
		Dart t;
		map.foreach_dart_until([&t] (Dart d) { t = d; return false; });
		const double area = cgogn::geometry::triangle_area<StdArray>(map, t, vertex_position);
		const double cf_area = cgogn::geometry::convex_face_area<StdArray>(map, t, vertex_position);
	...
	}
	{
		...
		Dart t;
		map.foreach_dart_until([&t] (Dart d) { t = d; return false; });
		const double area = cgogn::geometry::triangle_area<EigenVec3d>(map, t, vertex_position);
		const double cf_area = cgogn::geometry::convex_face_area<EigenVec3d>(map, t, vertex_position);
		EXPECT_DOUBLE_EQ(area, 12.5);
		EXPECT_DOUBLE_EQ(cf_area, 12.5);
	}
}
...
{
	{
		...
		VertexAttributeHandler<StdArray> vertex_position = map.get_attribute<StdArray, CMap2::VERTEX>("position");
		Dart q;
		map.foreach_dart_until([&q] (Dart d) { q = d; return false; });
		const double area = cgogn::geometry::convex_face_area<StdArray>(map, q, vertex_position);
	...
	}
	{
		...
		Dart q;
		map.foreach_dart_until([&q] (Dart d) { q = d; return false; });
		const double area = cgogn::geometry::convex_face_area<EigenVec3d>(map, q, vertex_position);
		EXPECT_DOUBLE_EQ(area, 10);
	}
}
...
{
	{
		...
		Dart t;
		map.foreach_dart_until([&t] (Dart d) { t = d; return false; });
		const StdArray centroid = cgogn::geometry::centroid<StdArray>(map, cgogn::Cell<CMap2::FACE>(t), vertex_position);
...

	{
		...
		Dart t;
		map.foreach_dart_until([&t] (Dart d) { t = d; return false; });
		const EigenVec3d centroid = cgogn::geometry::centroid<EigenVec3d>(map, cgogn::Cell<CMap2::FACE>(t), vertex_position);
		EXPECT_DOUBLE_EQ(centroid[0], 5.0/3.0);
		EXPECT_DOUBLE_EQ(centroid[1], 5.0/3.0);
		EXPECT_DOUBLE_EQ(centroid[2], 0);
	}
...
{
	{
		...
		Dart q;
		map.foreach_dart_until([&q] (Dart d) { q = d; return false; });
		const StdArray centroid = cgogn::geometry::centroid<StdArray>(map, cgogn::Cell<CMap2::FACE>(q), vertex_position);
...

	{
		...
		Dart q;
		map.foreach_dart_until([&q] (Dart d) { q = d; return false; });
		const EigenVec3d centroid = cgogn::geometry::centroid<EigenVec3d>(map, cgogn::Cell<CMap2::FACE>(q), vertex_position);
		EXPECT_DOUBLE_EQ(centroid[0], 2.5);
		EXPECT_DOUBLE_EQ(centroid[1], 1);
		EXPECT_DOUBLE_EQ(centroid[2], 0);
	}
...
{
	{
		...
		Dart t;
		map.foreach_dart_until([&t] (Dart d) { t = d; return false; });
		const StdArray& n1 = cgogn::geometry::triangle_normal<StdArray>(map, cgogn::Cell<CMap2::FACE>(t), vertex_position);
		const StdArray& n2 = cgogn::geometry::face_normal<StdArray>(map, cgogn::Cell<CMap2::FACE>(t), vertex_position);
	...
	}
	{
		...
		Dart t;
		map.foreach_dart_until([&t] (Dart d) { t = d; return false; });
		const EigenVec3d& n1 = cgogn::geometry::triangle_normal<EigenVec3d>(map, cgogn::Cell<CMap2::FACE>(t), vertex_position);
		const EigenVec3d& n2 = cgogn::geometry::face_normal<EigenVec3d>(map, cgogn::Cell<CMap2::FACE>(t), vertex_position);
		EXPECT_TRUE(cgogn::almost_equal_relative(n1[0], n2[0]));
		EXPECT_TRUE(cgogn::almost_equal_relative(n1[1], n2[1]));
		EXPECT_TRUE(cgogn::almost_equal_relative(n1[2], n2[2]));
...
{
	{
		...
		Dart q;
		map.foreach_dart_until([&q] (Dart d) { q = d; return false; });
		const StdArray& n1 = cgogn::geometry::face_normal<StdArray>(map, cgogn::Cell<CMap2::FACE>(q), vertex_position);
		const StdArray& cross = n1.cross(StdArray(0.,0.,1.));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[0], 0.));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[1], 0.));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[2], 0.));
	}
	{
		...
		Dart q;
		map.foreach_dart_until([&q] (Dart d) { q = d; return false; });
		const EigenVec3d& n1 = cgogn::geometry::triangle_normal<EigenVec3d>(map, cgogn::Cell<CMap2::FACE>(q), vertex_position);
		const EigenVec3d& cross = n1.cross(EigenVec3d(0,0,1));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[0], 0.));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[1], 0.));
		EXPECT_TRUE(cgogn::almost_equal_relative(cross[2], 0.));
