...
public:
	...
	/**
     * \brief Tests if all darts of the given orbit have the same embedding
     *
     * \tparam ORBIT [description]
     * \return [description]
     */
	template <Orbit ORBIT>
    bool is_well_embedded(Cell<ORBIT> c)
    {
    	...
    	if(emb_set.size() > 1)
    	{
        	...
        	for (it = emb_set.begin(); it != emb_set.end(); it++)
        	{
            	std::cout << "\t dart #" << (*it).second << " with embed index #" << (*it).first << "." << std::endl;
            }
            std::cout << std::endl;
        }
