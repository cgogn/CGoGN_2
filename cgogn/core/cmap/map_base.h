...
public:
	...
	bool is_well_embedded()
	{
		...
		AttributeHandler<uint32, ORBIT> counter = add_attribute<uint32, ORBIT>("__tmp_marker");
		bool result = true;
		...
		foreach_cell_until<FORCE_DART_MARKING>([&] (CellType c)
		{
			uint32 idx = this->get_embedding(c);
